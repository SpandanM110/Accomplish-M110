...
import { isSimpleConversationalPrompt } from '../opencode/electron-options';
...
export function registerIPCHandlers(): void {
  ...
  handle('task:start', async (event: IpcMainInvokeEvent, config: TaskConfig) => {
    ...
    // Direct chat path: bypass OpenCode for simple prompts - faster, more reliable (especially for local Ollama)
    const useDirectChat =
      isSimpleConversationalPrompt(validatedConfig.prompt) &&
      selectedModel?.model &&
      selectedModel?.provider;

    if (useDirectChat && selectedModel) {
      const apiKey =
        selectedModel.provider === 'ollama' || selectedModel.provider === 'lmstudio'
          ? undefined
          : getApiKey(selectedModel.provider);

      const directConfig = getDirectChatConfig(
        selectedModel.provider,
        selectedModel.model,
      ...
      if (directConfig && (selectedModel.provider === 'ollama' || selectedModel.provider === 'lmstudio' || apiKey)) {
        ...
        generateTaskSummary(validatedConfig.prompt, getApiKey, {
          ollama:
            selectedModel.provider === 'ollama' && selectedModel.baseUrl && selectedModel.model
              ? {
                  baseUrl: selectedModel.baseUrl,
                  model: selectedModel.model.replace(/^ollama\//, ''),
                }
              : undefined,
        })
          .then((summary) => {
          ...
          .catch((err) => console.warn('[IPC] Failed to generate task summary:', err));
    ...
    generateTaskSummary(validatedConfig.prompt, getApiKey, { ollama: summarizerOllama })
      .then((summary) => {
        storage.updateTaskSummary(taskId, summary);
        if (!window.isDestroyed() && !sender.isDestroyed()) {
          sender.send('task:summary', { taskId, summary });
  ...
  handle(
    ...
    async (
      ...
      // Direct chat session: use runDirectChat with conversation history for continued chat
      if (
        isDirectChatSession(validatedSessionId) &&
        validatedExistingTaskId &&
        selectedModelForResume?.model &&
        selectedModelForResume?.provider
      ) {
        const existingTask = storage.getTask(validatedExistingTaskId);
        if (!existingTask) {
          throw new Error(`Task ${validatedExistingTaskId} not found`);
        }

        const apiKey =
          selectedModelForResume.provider === 'ollama' ||
          selectedModelForResume.provider === 'lmstudio'
            ? undefined
            : getApiKey(selectedModelForResume.provider);

        const directConfig = getDirectChatConfig(
          selectedModelForResume.provider,
          selectedModelForResume.model,
          selectedModelForResume.baseUrl,
          apiKey ?? undefined,
        );
  ...
  handle('logs:export', async (event: IpcMainInvokeEvent) => {
    ...
    try {
      if (fs.existsSync(logPath)) {
        ...
        const header = `Accomplish M110 Application Logs\nExported: ${new Date().toISOString()}\nLog Directory: ${logDir}\n\nNo logs recorded yet.\n`;
