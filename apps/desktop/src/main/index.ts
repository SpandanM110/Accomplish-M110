...
const APP_DATA_NAME = 'Accomplish M110';
...
// GPU settings for Windows stability (GTX 1650 and similar)
// Disable hardware acceleration by default to prevent sudden crashes; set ACCOMPLISH_GPU=1 to enable
if (process.platform === 'win32') {
  if (process.env.ACCOMPLISH_GPU !== '1') {
...
import { disposeTaskManager, cleanupVertexServiceAccountKey, prewarmOpenCodeConfig } from './opencode';
...
// Reset data: relaunch passes this flag; new process deletes folder before any files are opened
if (process.argv.includes('--reset-data')) {
  const userDataPath = app.getPath('userData');
  console.log('[Reset] Deleting userData:', userDataPath);
  try {
    if (fs.existsSync(userDataPath)) {
      fs.rmSync(userDataPath, { recursive: true, force: true });
      console.log('[Reset] Successfully cleared userData');
    }
  } catch (err) {
    console.error('[Reset] Failed to delete:', err);
  }
}
...
// Log renderer crashes to help diagnose sudden closes
app.on('render-process-gone', (_event, _webContents, details) => {
  console.error('[Main] Renderer process gone:', details.reason, details.exitCode);
  try {
    getLogCollector().log('ERROR', 'main', `Renderer crashed: ${details.reason} (exit ${details.exitCode})`, {
      reason: details.reason,
      exitCode: details.exitCode,
    });
  } catch {
    // Log collector may not be ready
  }
});
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    try {
      ...
      if (err instanceof FutureSchemaError) {
        const { response } = await dialog.showMessageBox({
          ...
          message: `This data was created by a newer version of Accomplish M110 (schema v${err.storedVersion}).`,
          detail: `Your app supports up to schema v${err.appVersion}. You can reset your data to start fresh, or quit to update Accomplish M110.`,
          buttons: ['Reset data and start fresh', 'Quit'],
          defaultId: 0,
          cancelId: 1,
        });
        if (response === 0) {
          // Relaunch with --reset-data; the new process will delete the folder before opening any files
    ...
    if (mainWindow) {
      ...
      if (!(global as Record<string, unknown>).E2E_SKIP_AUTH && process.env.NODE_ENV !== 'test') {
        prewarmOpenCodeConfig();
      }
    }
