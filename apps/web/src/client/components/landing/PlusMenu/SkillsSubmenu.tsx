...
import { Trophy, Search, Lightbulb, ListChecks, Mic, FileText } from 'lucide-react';
import accomplishFavicon from '/assets/accomplish-favicon.png';

const HACKATHON_ACTIONS: { key: string; icon: typeof Search }[] = [
  { key: 'search', icon: Search },
  { key: 'validate', icon: Lightbulb },
  { key: 'ticketBoard', icon: ListChecks },
  { key: 'judgeDossiers', icon: Mic },
  { key: 'readme', icon: FileText },
  { key: 'winningProjects', icon: Trophy },
];
      ...
      <div className="max-h-[300px] overflow-y-auto">
        {filteredSkills.length === 0 ? (
          ...
          filteredSkills.map((skill) => {
            const isHackathonBuddy = skill.command === '/hackathon-buddy';
            return isHackathonBuddy ? (
              <DropdownMenuSub key={skill.id}>
                <DropdownMenuSubTrigger className="w-full px-3 py-2 cursor-pointer">
                ...
                <DropdownMenuSubContent side="right" align="start" className="w-[260px]">
                  {HACKATHON_ACTIONS.map(({ key, icon: Icon }) => (
                    <DropdownMenuItem
                      key={key}
                      onClick={() =>
                        onSkillSelect(
                          `/hackathon-buddy ${tHome(`hackathonSection.prompts.${key}`)}`,
                        )
                      }
                      className="flex items-start gap-2 py-2"
                    >
                      <Icon className="h-4 w-4 mt-0.5 shrink-0 text-amber-500" />
                      <span className="text-[12px]">
                        {tHome(`hackathonSection.prompts.${key}`)}
                      </span>
                    </DropdownMenuItem>
                  ))}
                </DropdownMenuSubContent>
