...
export function PlusMenu({ onSkillSelect, onOpenSettings, disabled }: PlusMenuProps) {
  ...
  useEffect(() => {
    if (open && window.accomplish) {
      window.accomplish
        ...
        .catch((err) => showError(err instanceof Error ? err.message : 'Failed to load skills'));

      window.accomplish
        .getConnectors()
        .then(setConnectors)
        .catch((err) => showError(err instanceof Error ? err.message : 'Failed to load connectors'));
    }
  }, [open, showError]);
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      showError(err instanceof Error ? err.message : 'Failed to refresh skills');
    } finally {
