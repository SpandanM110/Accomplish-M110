        ...
        {isComplete && !canFollowUp && (
          <div className="flex-shrink-0 border-t border-border bg-card/50 px-6 py-4 text-center">
            {currentTask.status === 'failed' && currentTask.result?.error && (
              <div className="mb-3 rounded-lg border border-destructive/30 bg-destructive/5 px-4 py-3 text-left">
                <p className="text-sm font-medium text-destructive mb-1">{t('status.failed')}</p>
                <p className="text-sm text-muted-foreground break-words">
                  {typeof currentTask.result.error === 'string'
                    ? currentTask.result.error
                    : (currentTask.result.error as { message?: string })?.message ||
                      String(currentTask.result.error)}
                </p>
              </div>
            )}
