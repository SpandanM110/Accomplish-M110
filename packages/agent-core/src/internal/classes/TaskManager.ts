...
import { AgentAdapter } from '../../agent/agent-adapter.js';
import type {
  TaskConfig,
  Task,
  TaskMessage,
...
export class TaskManager {
  ...
  private async executeTask(
    ...
    const opts = this.options.adapterOptions;
    const useAgentAdapter = opts.useAgentAdapter && opts.getAgentConfig;
      ...
      ? new AgentAdapter(
          {
            ...
            tempPath: this.options.defaultWorkingDirectory,
            getAgentConfig: opts.getAgentConfig!,
            onBeforeStart: opts.onBeforeStart,
            getModelDisplayName: opts.getModelDisplayName,
          },
          taskId,
        )
      : new OpenCodeAdapter(
          {
            ...opts,
            getCliCommand: opts.getCliCommand!,
            buildEnvironment: opts.buildEnvironment!,
            buildCliArgs: (taskConfig) => opts.buildCliArgs!(taskConfig, taskId),
          } as AdapterOptions,
          taskId,
        );
