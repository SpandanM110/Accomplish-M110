...
export async function startDevBrowserServer(
  ...
  const maxWaitMs = process.platform === 'win32' ? 20000 : 12000;
  const pollIntervalMs = process.platform === 'win32' ? 300 : 400;
  console.log(`[Browser] Waiting for dev-browser server (max ${maxWaitMs}ms, poll ${pollIntervalMs}ms)...`);

  const serverReady = await waitForDevBrowserServer(
    config.devBrowserPort,
    maxWaitMs,
    pollIntervalMs,
  );
