...
/** Adapter options for the underlying CLI adapter (OpenCode) or AI SDK AgentAdapter */
export interface TaskAdapterOptions {
  ...
  tempPath: string;
  /** Function to get the CLI command and arguments (required when useAgentAdapter is false) */
  getCliCommand?: () => { command: string; args: string[] };
  /** Function to build environment variables for a task (required when useAgentAdapter is false) */
  buildEnvironment?: (taskId: string) => Promise<NodeJS.ProcessEnv>;
  /** Function to build CLI arguments for a task (required when useAgentAdapter is false) */
  buildCliArgs?: (config: TaskConfig, taskId: string) => Promise<string[]>;
  /** Called before the adapter starts */
  onBeforeStart?: () => Promise<void>;
  /** Function to get display name for a model ID */
  getModelDisplayName?: (modelId: string) => string;
  /** When true, use AI SDK AgentAdapter instead of OpenCode CLI */
  useAgentAdapter?: boolean;
  /** Required when useAgentAdapter is true - builds agent config for each task */
  getAgentConfig?: (config: TaskConfig, taskId: string) => Promise<import('../agent/agent-config.js').AgentConfig>;
}
...
export interface TaskManagerOptions {
  ...
  /** Called before a task starts. Receives task config (prompt) for prompt-aware setup (e.g. skip browser for simple chat). */
  onBeforeTaskStart?: (
