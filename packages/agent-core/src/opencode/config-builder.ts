...
export async function buildProviderConfigs(
  ...
  if (groqApiKey) {
    let groqModelId: string | null = null;
    if (
      groqProvider?.connectionStatus === 'connected' &&
      groqProvider.selectedModelId
    ) {
      groqModelId = groqProvider.selectedModelId.replace(/^groq\//, '');
    } else if (activeModel?.provider === 'groq' && activeModel.model) {
      groqModelId = activeModel.model.replace(/^groq\//, '');
    }
    if (groqModelId) {
      providerConfigs.push({
        id: 'groq',
        ...
        models: {
          [groqModelId]: { name: groqModelId, tools: true },
        },
      });
      console.log('[OpenCode Config Builder] Groq configured:', groqModelId);
    }
  }
  ...
  if (activeModel?.provider === 'groq' && activeModel.model) {
    modelOverride = {
      model: activeModel.model,
      smallModel: activeModel.model,
    };
    console.log('[OpenCode Config Builder] Groq model override:', modelOverride);
  }
