...
You are Accomplish M110, a {{AGENT_ROLE}} assistant.
</identity>
...
`;
  ...
  const isConversationalMode = browserConfig.mode === 'none';

  if (isConversationalMode) {
    // Use minimal prompt for simple chat to stay under token limits (e.g. Groq free tier 6k TPM).
    // Full prompt with skills, file rules, etc. would exceed limits for "hi" / "hello" type messages.
    const envBrief = getPlatformEnvironmentInstructions(platform);
    systemPrompt = `<identity>
You are Accomplish M110, a task automation assistant.
</identity>

${envBrief}

<critical>
CONVERSATIONAL MODE - short user message (greeting, quick question).
- Call start_task with needs_planning: false, original_request: the user message, goal/steps/verification: omit.
- Respond with a brief, natural reply. One or two sentences.
- NEVER output JSON, schemas, examples, or "Here is a sample...". Just reply in plain text.
- Do NOT call complete_task. Respond and stop.
</critical>`;
  }
