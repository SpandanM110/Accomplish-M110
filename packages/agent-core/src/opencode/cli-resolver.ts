...
export function resolveCliPath(config: CliResolverConfig): ResolvedCliPaths | null {
  ...
  if (process.platform === 'win32') {
    // In pnpm workspaces, opencode-windows-x64 may be in appPath or workspace root (parent of appPath)
    const searchPaths = appPath ? [appPath, path.join(appPath, '..', '..')] : [];
    const localExeCandidates = searchPaths.flatMap((base) => getWindowsNodeModulesExeCandidates(base));
