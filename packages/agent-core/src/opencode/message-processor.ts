...
/** Hallucinated JSON/schema blocks from small models - strip from display */
const HALLUCINATED_JSON_BLOCK_RE =
  /\n*(?:Here is a sample JSON response|This JSON response defines|implements the functions specified)[\s\S]*$/gi;

/** JSON blocks that look like hallucinated tool schemas (e.g. {"name":"task","parameters":{...}}) */
const HALLUCINATED_TOOL_JSON_RE =
  /(?:^|\n)\s*\{[\s\S]*?"(?:original_request_summary|needs_planning|parameters)"\s*:[\s\S]*?$/gm;

/** JSON schema fragments - model outputs "type":"string", "description", "options" etc. */
const HALLUCINATED_SCHEMA_RE =
  /(?:^|\n)\s*(?:"type"\s*:\s*"string"|"description"\s*:\s*"[^"]*"|"options"\s*:\s*\[|"read-only"|"default"\s*:)[\s\S]*?$/gm;

/** Internal/Go-style output or JSON schema with additionalProperties/items/properties */
const HALLUCINATED_INTERNAL_SCHEMA_RE =
  /(?:^|\n)\s*(?:\{object\s+<nil>|"additionalProperties"\s*:\s*false|"items"\s*:\s*\{[^}]*"properties"|"required"\s*:\s*\[)[\s\S]*?$/gm;
...
export function sanitizeAssistantTextForDisplay(text: string): string | null {
  ...
  result = result.replace(HALLUCINATED_JSON_BLOCK_RE, '');
  result = result.replace(HALLUCINATED_TOOL_JSON_RE, '');
  result = result.replace(HALLUCINATED_SCHEMA_RE, '');
  result = result.replace(HALLUCINATED_INTERNAL_SCHEMA_RE, '');
