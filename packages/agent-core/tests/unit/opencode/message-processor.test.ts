...
describe('sanitizeAssistantTextForDisplay', () => {
  ...
  it('strips hallucinated JSON example blocks from small models', () => {
    const text =
      'Hello!\n\nHere is a sample JSON response that implements the functions: {"name":"start_task"}';
    expect(sanitizeAssistantTextForDisplay(text)).toBe('Hello!');
  });

  it('returns null when entire message is hallucinated JSON block', () => {
    const text =
      'Here is a sample JSON response that implements the functions specified in the prompt:\n{"name":"start_task"}';
    expect(sanitizeAssistantTextForDisplay(text)).toBeNull();
  });

  it('strips hallucinated tool schema JSON with parameters/goal/skills', () => {
    const text =
      'Hello!\n\n{"name": "task", "parameters": {"goal":["x"],"needs_planning":"false"}}';
    expect(sanitizeAssistantTextForDisplay(text)).toBe('Hello!');
  });

  it('returns null when entire message is hallucinated tool schema JSON', () => {
    const text =
      '{"name": "task", "parameters": {"goal":["Sort files"],"needs_planning":"false"},"original_request_summary":"Provide info"}';
    expect(sanitizeAssistantTextForDisplay(text)).toBeNull();
  });

  it('strips hallucinated JSON schema fragments (type, description, options)', () => {
    const text =
      'Hello!\n\n"type": "string", "default": "/path/file.txt", "options": ["markdown", "text"]';
    expect(sanitizeAssistantTextForDisplay(text)).toBe('Hello!');
  });

  it('returns null when entire message is hallucinated schema fragment', () => {
    const text = '"type": "string", "description": "The tool to be used", "options": ["a","b"]';
    expect(sanitizeAssistantTextForDisplay(text)).toBeNull();
  });

  it('strips internal/Go-style schema output ({object <nil> or additionalProperties)', () => {
    const text =
      'Hi!\n\n{object <nil> <nil> [tasks] {"tasks":{"type":"array","items":{"properties":{}}}}';
    expect(sanitizeAssistantTextForDisplay(text)).toBe('Hi!');
  });

  it('returns null when entire message is internal schema output', () => {
    const text =
      '{object <nil> <nil> [tasks] {"tasks":{"type":"array","additionalProperties":false}}';
    expect(sanitizeAssistantTextForDisplay(text)).toBeNull();
  });
});
