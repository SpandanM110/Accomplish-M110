...
function getDataDir(): string {
  ...
  if (process.platform === 'darwin') {
    return join(homeDir, 'Library', 'Application Support', 'Accomplish M110', 'dev-browser');
  } else if (process.platform === 'win32') {
    return join(process.env.APPDATA || homeDir, 'Accomplish M110', 'dev-browser');
  } else {
    return join(homeDir, '.accomplish-m110', 'dev-browser');
  }
}
    ...
    if (info.mode === 'extension') {
      ...
      try {
        if (process.platform === 'win32') {
          ...
          if (match) {
            try {
              execSync(`taskkill /F /PID ${match[1]}`, { stdio: ['ignore', 'ignore', 'pipe'] });
            } catch {
...
try {
  if (process.platform === 'win32') {
    ...
    if (match) {
      ...
      try {
        execSync(`taskkill /F /PID ${pid}`, { stdio: ['ignore', 'ignore', 'pipe'] });
      } catch {
        // Process may have already exited
      }
    }
